...
contract StakingDinamicoGainovo {
    ...
    function calcularTasa() public view returns (uint256) {
        ...
        // Ajuste para mantener precisión - usar más decimales internamente
        uint256 factorPrecision = 1e18;
        
        // Cálculo con alta precisión
        uint256 tasa = (balanceRecompensas * 100 * factorPrecision * (10**decimalsStake)) / 
                      (totalStaked * (10**decimalsReward));
        
        return tasa > (100 * factorPrecision) ? (100 * factorPrecision) : tasa;
    }
    ...
    // Función CORREGIDA para calcular intereses
    function calcularIntereses(address _user, uint256 _indice) public view returns(uint256) {
        ...
        // Cálculo de intereses con alta precisión
        // tasaActual ya incluye factorPrecision (1e18)
        uint256 interesesBrutos = (s.cantidad * tasaActual * tiempo) / (100 * 365 days);
        
        // Ajustar por diferencia de decimales entre tokens
        if (decimalsReward > decimalsStake) {
            // Recompensa tiene más decimales -> multiplicar
            interesesBrutos = interesesBrutos * (10**(decimalsReward - decimalsStake));
        } else if (decimalsReward < decimalsStake) {
            // Recompensa tiene menos decimales -> dividir
            interesesBrutos = interesesBrutos / (10**(decimalsStake - decimalsReward));
        }
        
        // Dividir por el factor de precisión usado en calcularTasa()
        return interesesBrutos / 1e18;
    }
    ...
    // Función CORREGIDA para finalizar stake
    function _finalizarStake(address _user, uint256 _indice, address _destino) private {
        ...
        // Transferir el stake original de vuelta
        IERC20(tokenStake).safeTransfer(_destino, s.cantidad);
        
        // Transferir intereses si hay suficientes recompensas
        if (intereses > 0) {
            uint256 balanceRecompensas = IERC20(tokenRecompensa).balanceOf(address(this));
            if (balanceRecompensas >= intereses) {
                IERC20(tokenRecompensa).safeTransfer(_destino, intereses);
            } else {
                // Si no hay suficientes recompensas, transferir lo disponible
                if (balanceRecompensas > 0) {
                    IERC20(tokenRecompensa).safeTransfer(_destino, balanceRecompensas);
                    intereses = balanceRecompensas; // Actualizar para el evento
                } else {
                    intereses = 0; // No hay recompensas disponibles
                }
            }
        }
